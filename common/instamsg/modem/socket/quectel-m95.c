...
#define BYTES_POLL_RESP_HEADER      "+QIRD:"
...
static int setUpModemMinimal(SG_Socket *socket)
{
     ...
     */
    i++;
    commands[i].command = "AT+IPR=9600\r";
    commands[i].delimiter = OK_DELIMITER;
    commands[i].logInfoCommand = "Set-Baud-Rate";
     ...
     */
    i++
    commands[i].command = "ATE0&W\r";
    commands[i].delimiter = OK_DELIMITER;
    commands[i].logInfoCommand = "Stop-Echo";
    commands[i].successStrings[0] = OK_DELIMITER;
    commands[i].successStrings[1] = NULL;
    commands[i].commandInCaseNoSuccessStringPresent = NULL;
#endif
     ...
     */
    i++;
    commands[i].command = "AT+QIMODE?\r";
    commands[i].delimiter = OK_DELIMITER;
    commands[i].logInfoCommand = "Enable-Non-Transparent-Mode";
    commands[i].successStrings[0] = "+QIMODE: 0";
    commands[i].successStrings[1] = NULL;
    commands[i].commandInCaseNoSuccessStringPresent = "AT+QIMODE=0\r";


    /*
     */
    i++;
    commands[i].command = "AT+QIMUX?\r";
    commands[i].delimiter = OK_DELIMITER;
    commands[i].logInfoCommand = "Enable-Multi-Connection";
    commands[i].successStrings[0] = "+QIMUX: 1";
    commands[i].successStrings[1] = NULL;
    commands[i].commandInCaseNoSuccessStringPresent = "AT+QIMUX=1\r";


    /*
     */
    i++;
    commands[i].command = "AT+QINDI?\r";
    commands[i].delimiter = OK_DELIMITER;
    commands[i].logInfoCommand = "Enable-Data-Sync-Read";
    commands[i].successStrings[0] = "+QINDI: 1";
    commands[i].successStrings[1] = NULL;
    commands[i].commandInCaseNoSuccessStringPresent = "AT+QINDI=1\r";


    /*
     */
    i++;
    commands[i].command = "AT+QIDNSIP?\r";
    commands[i].delimiter = OK_DELIMITER;
    commands[i].logInfoCommand = "IP-Address-As-Server-Name";
    commands[i].successStrings[0] = "+QIDNSIP: 0";
    commands[i].successStrings[1] = NULL;
    commands[i].commandInCaseNoSuccessStringPresent = "AT+QIDNSIP=0\r";


    /*
     */
    i++;
    commands[i].command = "AT&W\r";
    commands[i].delimiter = OK_DELIMITER;
    commands[i].logInfoCommand = "Persist-Config";
    commands[i].successStrings[0] = OK_DELIMITER;
    commands[i].successStrings[1] = NULL;
    commands[i].commandInCaseNoSuccessStringPresent = NULL;
...
#if 0
    i++;
    commands[i].command = "AT+CLTS?\r";
    commands[i].delimiter = OK_DELIMITER;
    commands[i].logInfoCommand = "Check-GSM-Sync-CLTS";
    commands[i].successStrings[0] = "+CLTS: 1";
    commands[i].successStrings[1] = NULL;
    commands[i].commandInCaseNoSuccessStringPresent = NULL;
#endif

    i++;
    commands[i].command = "AT+IPR?\r";
    commands[i].delimiter = OK_DELIMITER;
    commands[i].logInfoCommand = "Check-GSM-Sync-IPR";
    commands[i].successStrings[0] = "+IPR: 9600";
    commands[i].successStrings[1] = NULL;
    commands[i].commandInCaseNoSuccessStringPresent = NULL;
...
static int enableGsmSync(SG_Socket *socket)
{
...
#if 0
    i++;
    commands[i].command = "AT+CLTS=1\r";
    commands[i].delimiter = OK_DELIMITER;
    commands[i].logInfoCommand = "Enable-GSM-Sync-CLTS";
    commands[i].successStrings[0] = OK_DELIMITER;
    commands[i].successStrings[1] = NULL;
    commands[i].commandInCaseNoSuccessStringPresent = NULL;
#endif
    ...
    commands[i].command = "AT&W\r";
    commands[i].delimiter = OK_DELIMITER;
    commands[i].logInfoCommand = "Persist-GSM-Sync";
    commands[i].successStrings[0] = OK_DELIMITER;
    commands[i].successStrings[1] = NULL;
    commands[i].commandInCaseNoSuccessStringPresent = NULL;
...
{
    int rc = FAILURE;
    char i = -1, cgdContIndex = -1, apnBearerIndex = -1;
     ...
     */
    i++;
    commands[i].command = "AT+QIFGCNT?\r";
    commands[i].delimiter = OK_DELIMITER;
    commands[i].logInfoCommand = "Set-Context";
    commands[i].successStrings[0] = "+QIFGCNT: 0";
    commands[i].successStrings[1] = NULL;
    commands[i].commandInCaseNoSuccessStringPresent = "AT+QIFGCNT=0\r";
    ...
    commands[i].command = "AT+CGDCONT?\r";
    commands[i].delimiter = OK_DELIMITER;
    commands[i].logInfoCommand = "GPRS-Context-Correctness";
    commands[i].successStrings[0] = (char*)sg_malloc(MAX_BUFFER_SIZE);
    if(commands[i].successStrings[0] == NULL)
    {
    ...
    {
        sg_sprintf(commands[i].successStrings[0],"1,\"IP\",\"%s\"", socket->gsmApn);
    }
    commands[i].successStrings[1] = NULL;
    commands[i].commandInCaseNoSuccessStringPresent = (char*)sg_malloc(MAX_BUFFER_SIZE);
    if(commands[i].commandInCaseNoSuccessStringPresent == NULL)
    {
    ...
    {
        sg_sprintf(commands[i].commandInCaseNoSuccessStringPresent, "AT+CGDCONT=1,\"IP\",\"%s\"\r",
                   socket->gsmApn);
    }
    ...
    if(commands[i].command == NULL)
    {
        sg_sprintf(LOG_GLOBAL_BUFFER, "Could not allocate memory for command for AT+QICSGP");
        error_log(LOG_GLOBAL_BUFFER);
    ...
    {
        sg_sprintf(commands[i].command, "AT+QICSGP=1,\"%s\"\r", socket->gsmApn);
    }
    commands[i].delimiter = OK_DELIMITER;
    commands[i].logInfoCommand = "Set-APN-Profile";
    commands[i].successStrings[0] = OK_DELIMITER;
    commands[i].successStrings[1] = NULL;
    commands[i].commandInCaseNoSuccessStringPresent = NULL;
...
exit:
    if(commands[apnBearerIndex].command)
        sg_free(commands[apnBearerIndex].command);

    if(commands[cgdContIndex].commandInCaseNoSuccessStringPresent)
        sg_free(commands[cgdContIndex].commandInCaseNoSuccessStringPresent);

    if(commands[cgdContIndex].successStrings[0])
        sg_free(commands[cgdContIndex].successStrings[0]);
...
void quectel_m95_get_latest_sms_containing_substring(SG_Socket *socket, char *buffer, const char *substring)
{
    int smsIndex = 1;
...
static int setUpModemSocketUDP(SG_Socket *socket)
{
    ...
    /*
     */
    commands[0].command = "AT\r";
    commands[0].delimiter = OK_DELIMITER;
    commands[0].logInfoCommand = "Dummy-AT-Commmand";
    ...
    {
        sg_sprintf(LOG_GLOBAL_BUFFER, MODEM_SOCKET "Could not allocate memory for AT+QIOPEN", socket->socket);
        error_log(LOG_GLOBAL_BUFFER);
    ...
    sg_sprintf(commands[1].command,
               "AT+QIOPEN=%u,\"%s\",\"%s\",%u\r", socket->socket, socket->type, socket->host, socket->port);
 ...
 */
void quectel_m95_connect_underlying_socket_medium_try_once(SG_Socket* socket)
{
    int rc = FAILURE;
...
int quectel_m95_socket_read(SG_Socket* socket, unsigned char* buffer, int len, unsigned char guaranteed)
{
    int bytesSoFar = 0;
    ...
    {
        memset(smallBuffer, 0, sizeof(smallBuffer));
        sg_sprintf(smallBuffer, "AT+QIRD=0,2,%u,%d\r", socket->socket, len - bytesSoFar);
...
int quectel_m95_socket_write(SG_Socket* socket, unsigned char* buffer, int len)
{
    memset(smallBuffer, 0, sizeof(smallBuffer));
    sg_sprintf(smallBuffer, "AT+QISEND=%u,%d\r", socket->socket, len);
 ...
 */
void quectel_m95_release_underlying_socket_medium_guaranteed(SG_Socket* socket)
{
    waitBeforeReboot();
    resetDevice();
}
 ...
 */
void quectel_m95_load_client_certificate_into_buffer(char *cert_buffer, int maxLength)
{
}
 ...
 */
void quectel_m95_save_client_certificate_from_buffer(char *cert_buffer)
{
}
 ...
 */
void quectel_m95_load_client_private_key_into_buffer(char *private_key_buffer, int maxLength)
{
}
 ...
 */
void quectel_m95_save_client_private_key_from_buffer(char *private_key_buffer)
{
}
